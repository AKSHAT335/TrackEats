...
const CheckOutMap = dynamic(
  () => import("../../components/CheckoutMap").then((mod) => mod.default),
  { ssr: false },
);
...
function Checkout() {
  ...
  const handleOnlinePayment = async () => {
    ...
    const userId = session.user.id; // FIXED HERE
    ...
    try {
      ...
      const { razorpayOrderId, amount, currency } = result.data;
      ...
      const options = {
        ...
        handler: async function (response: any) {
          // verify payment
          const verifyRes = await axios.post("/api/user/verify-payment", {
            ...response,
            userId: userId,
            items: cartData.map((item) => ({
              grocery: item._id,
