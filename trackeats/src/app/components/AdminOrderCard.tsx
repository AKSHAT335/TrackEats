...
function AdminOrderCard({ order }: { order: IOrder }) {
  ...
  useEffect((): any => {
    ...
    // Setup listener immediately if connected, or wait for connection
    if (socket.connected) {
      setupListener();
    } else {
  ...
  return (
    ...
    >
      ...
      {order.assignedDeliveryBoy && (
        <div className="mt-4 w-full bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center justify-between">
          ...
          <a
            ...
            className="shrink-0 bg-blue-600 text-white text-xs px-4 py-2 rounded-lg hover:bg-blue-700 transition"
          >
            Call
          </a>
        </div>
